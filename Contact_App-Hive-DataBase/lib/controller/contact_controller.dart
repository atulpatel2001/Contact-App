...
class ContactController extends GetxController {
  ...
  void reflectFilterCategoryFunction() {
    loadContacts();
    try {
  ...
  void loadContacts() async {
    try {
      var box = Hive.box("contact");
      List<Contact> lists = box.values.cast<Contact>().toList();
      contacts.value = lists;
    } catch (e) {
      printInfo(info: e.toString());
    }
  }
   ...
   * @Return void
   */
  void updateContact(index, Contact contact) async {
    try {
      var box = Hive.box("contact");

      Contact oldContact = box.getAt(index)!;
      ...
      oldContact.category = contact.category;
      await box.putAt(index, oldContact);
     loadContacts();
    } catch (e) {
      printInfo(info: e.toString());
    }
   ...
   * @Return Type :-void
   */
  void searchContacts(String searchString) {
    try {
      if (searchString.isEmpty) {
        contacts.assignAll(getContacts());
      } else {
        contacts.assignAll(getContacts().where((contact) =>
            (contact.firstName ?? '')
                .toLowerCase()
